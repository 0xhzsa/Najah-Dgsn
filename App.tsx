  ...
  return (
    <div style={{
      ...
      <div className="font-bold text-lg md:text-xl tracking-tight flex items-center gap-2">
        ...
        <span>الامتحان يوم 7 دجنبر</span>
...
const App = () => {
  ...
  const cancelQuiz = () => {
      ...
      setMode(AppMode.HOME);
      // Wait for state to clear before showing modal
  ...
  const handleGenerateMore = async () => {
    ...
    try {
      ...
      if (mode === AppMode.QUIZ) {
          ...
          // Insert after current question so user sees it next
  ...
  const renderQuiz = () => {
    const currentQ = quizSessionQuestions[currentQIndex];
    // Safety Check: ensure currentQ exists before rendering
